...
- **Upload path:** `POST /api/uploads` (Tina-authenticated via `assertTinaAuthorized`, same as Insights) → `prepareDownloadUpload()` in `lib/checkout` → `createSignedUploadUrl` into `product-downloads` → browser PUTs directly to Supabase. Allowed: pdf/zip/epub. Object path = `<product-name-slug>/<file-slug>.<ext>`, upsert on.
- **Vercel Hobby 12-function cap:** the keep-alive cron had already pushed `api/` to 13 files and every deploy since 13:27 on 2026-09-06 was failing with `exceeded_serverless_functions_per_deployment`. Consolidated to 12: `/api/uploads` lives in `api/insights.ts` (`__route=uploads`) and `/api/checkout/success` in `api/checkout.ts` (`__route=success`), both via `vercel.json` rewrites — public URLs unchanged. **Adding any new `api/*.ts` file will break deploys again**; fold new routes into an existing function the same way.
